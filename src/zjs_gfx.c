...
static jerry_value_t zjs_gfx_draw_char_priv(u32_t x, u32_t y, char c, u8_t color[], u32_t size, gfx_handle_t *gfxHandle)
{
    ...
    u8_t fontBytes = font_data_descriptors[asciiIndex][0] * 2;  // 2 bytes per character
...
static ZJS_DECL_FUNC(zjs_gfx_draw_string)
{
    ...
    for (u8_t i = 0; i < argData.textSize; i++) {
        ...
        //ret = zjs_gfx_draw_char_priv(argData.coords[0] + (i * charWidth * argData.size), argData.coords[1], argData.text[i], argData.color, argData.size, handle);
